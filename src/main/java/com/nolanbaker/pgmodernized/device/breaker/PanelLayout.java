 ...
 * <p>
 * Terminals: 0 line, 1 neutral, 2.. circuits; then the 8 hubs; then 12 hidden landings per hub.
 * The asset generator {@code tools/gen_breaker_panel_assets.py} carries the same numbers; change both.
 */
public final class PanelLayout {
    ...
    public static int hubBase(PanelSpec spec) {
        return BreakerPanelBlockEntity.TERMINAL_BRANCH_FIRST + spec.slots();
    }
    ...
    private static double[] rect(PanelSpec spec, int slot) {
        return slot == MAIN ? mainRect() : slotRect(spec, slot);
    }

    public static AABB breakerBox(PanelSpec spec, int slot) {
        var r = rect(spec, slot);
        double u1 = r[0] + VISUAL_INSET, u2 = r[2] - VISUAL_INSET;
        return new AABB(16 - u2, r[1], FRONT_Z - BREAKER_DEPTH, 16 - u1, r[3], FRONT_Z);
    }

    public static Vec3 breakerCenter(PanelSpec spec, int slot) {
        var box = breakerBox(spec, slot);
    ...
    public static TerminalBoundingBox[] terminals(PanelSpec spec) {
        ...
        terminals[BreakerPanelBlockEntity.TERMINAL_LINE] = terminal(name("breaker_panel.line", ChatFormatting.RED), HIDDEN).withColor(IDecoratedTerminal.RED);
        terminals[BreakerPanelBlockEntity.TERMINAL_NEUTRAL] = terminal(name("breaker_panel.neutral", ChatFormatting.BLUE), HIDDEN).withColor(IDecoratedTerminal.BLUE);
        for(int slot = 0; slot < spec.slots(); ++slot) {
            ...
            terminals[BreakerPanelBlockEntity.TERMINAL_BRANCH_FIRST + slot] = terminal(label, HIDDEN).withColor(IDecoratedTerminal.GRAY);
        }
        for(int h = 0; h < HUB_COUNT; ++h) {
